# сравнение кортежей через хеш и создание полного списка пар, троек, четверок, и т. д.
...
start1 = clock()
z=0
d=(99999, 99999, 99999, 99999, 99999) # как-то тут регулируется количество элементов в кортеже,
                                      # если так  (99999, 99999, 99999) то будет три элемента
...
def lst_tupl(): # функция создает список кортежей
    #nambers=f
    lst=[]
    squared_odds = [0,0,0,0,0]# на самом деле это не где девятки, а здесь регулируется длина кортежа
    for k in range(36):
        start1 = clock()
        time.sleep(0.001)
        squared_odds[0]=k
